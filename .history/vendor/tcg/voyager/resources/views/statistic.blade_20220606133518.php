                ...
                <div class="row">
                    <div class="col-md-3" style="width: 20%">
                        ...
                        <select name="created_at" class="form-control" style=" height:35px ; width:196px"
                            id="dropdown1">
                            @php
                             // $tables = 'SHOW TABLES FROM gest_archivage';  
                             $database_name = "gest_archivage"; 
                             $tables = DB::select("SELECT table_name FROM information_schema.tables WHERE table_schema = '{$database_name}'  SHOW TABLES LIKE 'branches' ");               
                            @endphp
                                <option disabled selected>All</option>
                                @foreach($tables as $key => $table)
                                    <option>{{ $table_names[] = $table->table_name echo $table->Tables_in_db_name; }}</option>
                                @endforeach
