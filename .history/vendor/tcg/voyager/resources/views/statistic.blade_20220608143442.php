...
@section('content')
@php
// $tables = DB::select('SHOW TABLES'); 2ème méthode  
$database_name = "gest_archivage"; 
 $array = "('branches', 'dossiers', 'visites', 'patients','services')"; 
$tables = DB::select("SELECT table_name FROM information_schema.tables WHERE table_schema = '{$database_name}' AND table_name in {$array} ");  
// var_dump($array);
$all = array();
foreach ($tables as $key => $table) {
    ...
    array_push($all, ['table'=> $table->table_name, 'columns' => $columns]);
} 
//dd($all);
//for each table
// search columns
 //   array_push($all , ['table' => $t , 'columns' =>$columns])
...
@endphp
    <div class="page-content">
        <body>
            ...
            <div class="col-md-12">
                <div class="row">
                    <div class="col-md-3" style="width: 20%">
                        ...
                        <select name="dropdown1" class="form-control" style=" height:35px ; width:196px"
                            id="dropdown1" onChange="showitems()" >
                       
                                <option disabled selected>All</option>
                                @foreach($tables as $key => $table)
                                    <option value="{{ $table->table_name }}">{{ $table_names[] = $table->table_name }}</option>
                                    {{--  <option>{{ $table->Tables_in_gest_archivage }}</option> 2ème méthode --}}
                                @endforeach$
                        </select>
                        <script type="text/javascript">
                            function showitems() {
                                ...
                                var ligne = all.find((objet)=>objet.table == option)
                                var columns = ligne.columns
                                //console.log(columns);

                                $(document).ready(function () {
                                    $.getJSON(function (showitems) {
                                        html = ''
                                        for (i = 0; i < results.length; i++) {
                                            html += "<option value=" + showitems[i] + ">" + showitems[i] + "</option>";
                                        };
                                        document.getElementById("dropdo").innerHTML = html;
                                    });
                                });
                            }
                        </script>
                    ...
                    <div class="col-md-3">
                        <label style="color: #000">Sélectionner Attribut : *</label><br>
                        <select name="attribut" class="form-control" style=" height:35px ; width:196px"
                            id="dropdown2">
                            @php
                                $tables = DB::select("SELECT table_name FROM information_schema.tables WHERE table_schema = '{$database_name}' ");  
                            @endphp
                                <option value="">Sélectionner Attribut : *</option>
                                @foreach($tables as $key => $table)
                                  <option>{{'SHOW SHOW COLUMNS FROM $table '}}</option>
                                @endforeach 
                        </select>
            ...
            <div class="col-md-12">
                <label for="huey">Selectionner Chart :</label><br>
                <div class="row" style="width: 51%">
                    <div class="col-md-3">
                        <input type="radio" id="Circ_Chart" name="Circ_Chart" value="Circ_Chart" onclick="checkRadio(name)">
                        <img class="Circular-Chart" src="{{ asset('/storage/Circular-Chart.jpg') }}" >                              
                        <label for="huey">Circular Chart</label>
                    </div>
                    <div class="col-md-3" >
                        <input type="radio" id="Rect_Chart" name="Rect_Chart" value="Rect_Chart" onclick="checkRadio(name)">
                        <img class="Rectangular-Chart" src="{{ asset('/storage/Rectangular-Chart.jpg') }}">  
                        <label for="huey">Rectangular Chart</label>
                    </div>
                    <div class="col-md-3">
                        <input type="radio" id="Line_Chart" name="Line_Chart" value="Line_Chart" onclick="checkRadio(name)">
                        <img class="Line-Chart" src="{{ asset('/storage/Line-Chart.png') }}">
                        <label for="huey">Line Chart</label>
                    </div>
                    ...
                    <script>  
                        function checkRadio(name) {
                            ...
                            console.log("Choice: ", name);
                                document.getElementById("Line_Chart").checked = true;
                                document.getElementById("Rect_Chart").checked = false;
                                document.getElementById("Circ_Chart").checked = false;

                            } else if (name == "Rect_Chart"){
                                console.log("Choice: ", name);
                                document.getElementById("Rect_Chart").checked = true;
                                document.getElementById("Circ_Chart").checked = false;
                                document.getElementById("Line_Chart").checked = false;

                            }else if (name == "Circ_Chart"){
                                console.log("Choice: ", name);
                                document.getElementById("Circ_Chart").checked = true;
                                document.getElementById("Rect_Chart").checked = false;
                                document.getElementById("Line_Chart").checked = false;
                            }
                        }
